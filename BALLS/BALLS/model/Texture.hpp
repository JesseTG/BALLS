...
class AbstractTexture : public QObject {
  ...
  // clang-format off
  Q_PROPERTY(QColor borderColor READ borderColor WRITE borderColor STORED false FINAL)
  Q_PROPERTY(QOpenGLTexture::DepthStencilMode depthStencilMode READ depthStencilMode WRITE depthStencilMode STORED false FINAL)
  // TODO: Register foreign enum types

  Q_PROPERTY(int faces READ faces STORED false FINAL)
  Q_PROPERTY(QOpenGLTexture::TextureFormat format READ format WRITE format STORED false FINAL)
  Q_PROPERTY(QOpenGLTexture::Filter magnificationFilter READ magnificationFilter WRITE magnificationFilter STORED false FINAL)
  Q_PROPERTY(QOpenGLTexture::Filter minificationFilter READ minificationFilter WRITE minificationFilter STORED false FINAL)
  Q_PROPERTY(float maxAnisotropy READ maxAnisotropy WRITE maxAnisotropy STORED false FINAL)
...
public /* getters/setters */:
  ...
  float maxAnisotropy() const noexcept {
    return texture_->maximumAnisotropy();
  }
  ...
  float maxLod() const noexcept {
    return texture_->maximumLevelOfDetail();
  }

  void maxLod(float lod) noexcept {
    texture_->setMaximumLevelOfDetail(lod);
  }

  float minLod() const noexcept {
    return texture_->minimumLevelOfDetail();
  }

  void minLod(float lod) noexcept {
    texture_->setMinimumLevelOfDetail(lod);
  }

  int samples() const noexcept {
    return texture_->samples();
  }

  void samples(int samples) noexcept {
    texture_->setSamples(samples);
  }

  QOpenGLTexture::Target target() const noexcept {
    return texture_->target();
  }
