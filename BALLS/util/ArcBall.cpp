...
void ArcBall::beginDrag(const glm::vec2& msc)
{
  ...
  // Normal 'begin' code.
  mVDown      = vec3(mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0f, 1.0));
}
...
void ArcBall::drag(const glm::vec2& msc)
{
  ...
  mVNow       = vec3(mScreenToTCS * glm::vec4(msc.x, msc.y, 0.0, 1.0));
...
//------------------------------------------------------------------------------
void ArcBall::setScreenToTCS(const glm::mat4 &screenToTCS) noexcept
{
  mScreenToTCS = screenToTCS;
}
...
//------------------------------------------------------------------------------
const glm::mat4& ArcBall::getTransformation() const
{
  return mMatNow;
}
