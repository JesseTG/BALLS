...
void ColorCombo::setColor(const QColor& color) noexcept {
  m_init = color;
...
void ColorCombo::currentChanged(const int index) noexcept {
  QVariant data = itemData(index);

  if (data.isValid() && data == static_cast<int>(QVariant::UserType)) {
    QColor color = QColorDialog::getColor(m_init, this);

    if (color.isValid()) {
      if (findData(color, static_cast<int>(Qt::DecorationRole)) == -1) {
        addItem(color.name());
        setItemData(count() - 1, color, Qt::DecorationRole);
      }
